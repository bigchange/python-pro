...
from bert_ner_new import parse_tfrecord_function, Model as NerModel
...
def parse_args():
    ...
    parser.add_argument('--warm_steps', default=200)
    parser.add_argument('--train_steps', default=4000)
...
def make_feed_dict(model, inputs, droprate=0):
    return {
        ...
        model.types_placeholder: inputs[3],
...
def test_eval(sess, unary_score, test_squence_length,
    ...
    for i in range(numbatch):
        ...
        types = [b[3] for b in data]
        inputs = (sentence, targets, lengths, types)
        ...
        for unary_, y_, l_ in zip(unary_score_val, y, length):
            unary_ = unary_[:l_-1]
            y_ = y_[:l_-1]
            # preds = np.argmax(unary_, axis=1)
            preds, _ = tf.contrib.crf.viterbi_decode(unary_, transMatrix)
            correct_labels += np.sum(np.equal(preds, y_))
            total_labels += l_-1
        # print("max u=%f, min u=%f"%(maxU, minU))
...
def main(args):
    ...
    with graph.as_default():
        ...
        with sv.managed_session(master='') as sess:
            ...
            steps = 100000
            if args.dump_words:
                words=sess.run(model.bert.get_embedding_table())
                nn=len(words)
