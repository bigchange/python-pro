...
tf.app.flags.DEFINE_integer("vocab_size", 50003, "the vocab size")
...
def main(unused_argv):
    ...
    with tf.Graph().as_default():
        ...
        datasetTrain = filenames.apply(
            ...
            lambda filename: tf.data.TFRecordDataset(filename,compression_type='GZIP', buffer_size=4096*1024),
            cycle_length=10))
        ...
        datasetTrain = datasetTrain.map(
            model_use_official.parse_tfrecord_function)
        ...
        datasetTest = datasetTest.map(model_use_official.parse_tfrecord_function)
        iteratorTest = datasetTest.make_initializable_iterator()
        test_input = iteratorTest.get_next()
        model = model_use_official.Model(FLAGS.wordvec_size,FLAGS.vocab_size)
