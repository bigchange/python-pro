...
def partial_transfer(checkpointPath, igorePattern=None):
    ...
    for v in allVars:
        ...
        lastIdx = name.rfind(":")
        name = name[:lastIdx]
        if name.startswith("cls/title_out") or name.find("global_step")!= -1 or (igorePattern is not None and  name.find(igorePattern)!= -1 ):
          print("======================skip %s....."%(name))
          continue
