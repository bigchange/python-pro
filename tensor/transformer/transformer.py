...
class Transformer(object):
    ...
    def __init__(self, params, train):
        ...
        self.EOS_ID = params['vocab_size']-2

    def __call__(self, inputs, input_types, targets=None, encodeAttMask=None):
        ...
        with tf.variable_scope("Transformer", reuse=tf.AUTO_REUSE, initializer=initializer):
            # Calculate attention bias for encoder self-attention and decoder
            # multi-headed attention layers.
            attention_bias = model_utils.get_padding_bias(inputs)
            if encodeAttMask is not None:
                attention_bias = attention_bias + encodeAttMask
            # Run the inputs through the encoder layer to map the symbol
            # representations to continuous representations.
            encoder_outputs, pos_encoding = self.encode(inputs, attention_bias, input_types)

            # Generate output sequence if targets is None, or return logits if target
            # sequence is known.
            if targets is None:
                return self.predict(encoder_outputs, attention_bias), encoder_outputs, pos_encoding
            else:
                logits = self.decode(targets, encoder_outputs, attention_bias)
                return logits, encoder_outputs, pos_encoding
            ...
            with tf.name_scope("add_pos_encoding"):
                ...
                if input_types is not None:
                    input_types = model_utils.get_input_types(
                        input_types, self.params["hidden_size"], num_types=self.params["num_types"])
                    encoder_inputs = encoder_inputs + input_types
            if self.train:
                encoder_inputs = tf.nn.dropout(
                    encoder_inputs, 1 - self.params["layer_postprocess_dropout"])

            return self.encoder_stack(encoder_inputs, attention_bias, inputs_padding), pos_encoding
    ...
    def _get_symbols_to_logits_fn(self, max_decode_length):
        ...
        def symbols_to_logits_fn(ids, i, cache):
            ...
            decoder_outputs = self.decoder_stack(
                decoder_input, cache.get(
                    "encoder_outputs"), self_attention_bias,
