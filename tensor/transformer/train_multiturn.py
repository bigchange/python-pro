...
from multiturn_model import MultiTurnModel as Model
...
tf.app.flags.DEFINE_string('dev_data_path', "/data/multiturn/tf/dev.tfrecord",
                           'Dev data path')
tf.app.flags.DEFINE_string('log_dir', "multiturn_logs", 'The log  dir')
...
tf.app.flags.DEFINE_integer("track_history", 14, "track max history accuracy")
tf.app.flags.DEFINE_integer("warm_steps", 10000, "track max history accuracy")
...
def parse_tfrecord_function(example_proto):
    features = {
        "partIds": tf.FixedLenSequenceFeature([], tf.int64,
                                        allow_missing=True,
                                        default_value=0),
        "allIds": tf.FixedLenSequenceFeature([], tf.int64,
                                        allow_missing=True,
                                        default_value=0),
        "aLen": tf.FixedLenFeature([], tf.int64,
                                   default_value=0),
        "tLen": tf.FixedLenFeature([], tf.int64,
                                   default_value=0),
    ...
    parts = parsed_features["partIds"]
    parts.set_shape([FLAGS.max_length])
    alen = parsed_features["aLen"]
    tlen = parsed_features["tLen"]
    target = parsed_features["target"]
    return x, parts, alen, tlen, target
